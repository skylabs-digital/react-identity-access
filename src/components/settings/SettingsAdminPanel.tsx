...
export const SettingsAdminPanel: React.FC<SettingsAdminPanelProps> = ({
  ...
  // Sync local settings when context settings change (only on initial load)
  useEffect(() => {
    setLocalSettings((prevLocal: any) => {
      // Only update if local settings are empty or significantly different
      if (!prevLocal || Object.keys(prevLocal).length === 0) {
        return settings;
      }
      return prevLocal;
    });
  ...
  const { fieldMetadata } = SchemaAnalyzer.analyzeSchema(schema as any);
